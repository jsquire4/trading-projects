...
describe("binaryCallPrice", () => {
  const sigma = 0.30; // 30% annualized volatility
  const T = 6.5 / 8760; // 6.5 hours in years
  const r = 0.05;

  it("ATM (S = K): probability near 0.50", () => {
    const price = binaryCallPrice(100, 100, sigma, T, r);
    // Slightly above 0.50 due to risk-free drift, but close
  ...
  it("very small T (5 minutes), S well above K → near 0.99", () => {
    const fiveMinutes = 5 / (365.25 * 24 * 60); // 5 minutes in years
    const price = binaryCallPrice(110, 100, sigma, fiveMinutes, r);
    expect(price).toBeGreaterThan(0.95);
  });

  it("very small T (5 minutes), S well below K → near 0.01", () => {
  ...
  it("higher sigma increases uncertainty (ATM price stays near 0.50)", () => {
    // With very low vol, ATM should be near 0.50
    const lowVol = binaryCallPrice(100, 100, 0.01, T, r);
    // With very high vol, ATM should still be near 0.50 (symmetric around strike)
    const highVol = binaryCallPrice(100, 100, 2.0, T, r);
    // Both should be in a reasonable range around 0.50
    expect(lowVol).toBeGreaterThan(0.40);
    expect(lowVol).toBeLessThan(0.60);
    expect(highVol).toBeGreaterThan(0.30);
    expect(highVol).toBeLessThan(0.70);
  });
});
