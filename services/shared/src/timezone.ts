/**
 * ET Timezone Helpers — canonical location (shared)
 *
...
import { createLogger } from "./alerting.js";

const log = createLogger("shared:timezone");
 ...
 * Cache has a 1-hour TTL to prevent stale results across long-running processes.
 */
...
export async function isMarketDay(date: Date = new Date()): Promise<boolean> {
  ...
  const now = Date.now();
  if (dayKey === _cachedDayKey && _cachedResult !== null && (now - _cachedAt) < CACHE_TTL_MS) {
